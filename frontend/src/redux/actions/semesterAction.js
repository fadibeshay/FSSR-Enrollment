...
export const LoadSemester =
  ...
  async (dispatch, getState) => {
    try {
      ...
      if (
        !getState().semester.semester.semester._id ||
        getState().semester.search !== keyword ||
        getState().semester.semester.page !== page ||
        (!current && getState().semester.semester.semester._id !== id) ||
        current !== getState().semester.semester.current
      ) {
        dispatch({
        ...
        dispatch({
          ...
          payload: { semester: { ...data, current }, search: keyword }
        });
      }
