...
export const LoadSubjects =
  ...
  async (dispatch, getState) => {
    try {
      if (
        getState().subject.subjects.subjects.length === 0 ||
        getState().subject.subjects.subjects.length > pageSize ||
        !getState().subject.subjects.page ||
        getState().subject.search !== keyword ||
        getState().subject.subjects.page !== page
      ) {
        dispatch({
        ...
        dispatch({
          ...
          payload: { subjects: data, search: keyword }
        });
      }
...
export const LoadSubject = (_id) => async (dispatch, getState) => {
  try {
    const subject = getState().subject.subjects.subjects.find(
      (s) => s._id === _id
    );
    if (subject) {
      dispatch({
